...
<html lang="en">
<head>
  <title>Search Flights - SpanishFly</title>
<meta charset="utf-8">
...
global $sql1,$sql2,$result, $availableNumber;
  ...
  if($stop=="nonstop"){
    ...
    $sql1 = "SELECT FL.number AS FLnumber, company, type, departure, d_time, arrival, a_time, C.name AS classname, capacity, price, COUNT(*)
            ...
            WHERE (FL.number = C.number) AND (FL.airplane_id = AP.ID) AND C.name = '$class' AND FL.status='Approved' AND C.status='Approved' AND 
    ...
    $sql3 = "SELECT FL.number AS FLnumber, company, type, departure, d_time, arrival, a_time, C.name AS classname, capacity, price
           ...
           WHERE (FL.number = C.number) AND (FL.airplane_id = AP.ID) AND C.name = '$class' AND arrival = '$to') AND FL.status='Approved' AND C.status='Approved'
