...
$rowcount = mysqli_num_rows($result);
    ...
    echo "<div class='alert alert-info'>List of flights</div>";
    ...
    while($row = mysqli_fetch_array($result)) 
	{
		...
		echo '<td><form action="checkinConfirm.php" method="post">
			<input type="text" name="id" placeholder="Enter ticket number">
			<input type="hidden" name="flight" value="'.$flight.'" >
			<input type="hidden" name="date" value="'.$date.'" >
			<input type="hidden" name="arrival" value="'.$arrival.'" >
			<input type="hidden" name="departure" value="'.$departure.'" >
            <br><button type="submit" class="btn btn-warning">Check in</button></div>
			</form>
			</td>';
